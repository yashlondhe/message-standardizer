...
@Service
@Slf4j
public class AlphaMessageTransformationService implements MessageTransformationServiceInterface<AlphaMessage> {
    
    @Override
    public StandardMessage transform(AlphaMessage message) {
    ...
    private StandardOddsMessage transformOdds(AlphaOddsMessage alphaMessage) {
        ...
        standardMessage.setMessageType(Constants.StandardMessageType.ODDS_CHANGE.getValue());
        standardMessage.setClientMessageId(alphaMessage.getEventId());
    ...
    private StandardSettlementMessage transformSettlement(AlphaSettlementMessage alphaMessage) {
        ...
        standardMessage.setMessageId(MiscUtils.generateMessageId());
        standardMessage.setTimestamp(MiscUtils.getCurrentTimestamp());
        standardMessage.setMessageType(Constants.StandardMessageType.BET_SETTLEMENT.getValue());
        standardMessage.setClientMessageId(alphaMessage.getEventId());
